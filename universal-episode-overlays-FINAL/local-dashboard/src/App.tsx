...
import BetaBotControl from './components/BetaBotControl'
...
type TabType = 'dashboard' | 'live' | 'scenes' | 'sources' | 'media' | 'audio' | 'production' | 'graphics' | 'settings' | 'monitor' | 'advanced' | 'betabot'
...
function App() {
  ...
  const handleConnect = async (address: string, password: string) => {
    try {
      ...
      let fixedAddress = address.trim()
      if (!fixedAddress.startsWith('ws://') && !fixedAddress.startsWith('wss://')) {
        console.log('🔧 Auto-fixing protocol - adding ws:// prefix')
        fixedAddress = `ws://${fixedAddress}`
      }
      
      await obs.connect({ address: fixedAddress, password })
    } catch (error) {
          ...
          <div className="flex items-center gap-2">
            <div className="flex gap-2 mr-4 overflow-x-auto">
              {[
                ...
                { id: 'betabot', label: 'BetaBot', color: '#059669', icon: '🤖' },
