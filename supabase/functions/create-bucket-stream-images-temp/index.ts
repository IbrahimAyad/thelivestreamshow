...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/png", "image/gif", "image/webp"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for stream-images" ON storage.objects FOR SELECT USING (bucket_id = 'stream-images');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for stream-images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'stream-images');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for stream-images" ON storage.objects FOR UPDATE USING (bucket_id = 'stream-images');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for stream-images" ON storage.objects FOR DELETE USING (bucket_id = 'stream-images');`
    ];
