...
import { Search, Settings, Music2, Mic2, Download, Filter, Plus, Bot, BarChart3, Clock, Radio, Shield, Palette, Save, Music, Activity, Disc, Mic, Layers, Brain } from 'lucide-react'
...
import { useQuantize } from '@/hooks/studio/useQuantize'
import { useLoopRoll } from '@/hooks/studio/useLoopRoll'
import { usePlayQueue } from '@/hooks/studio/usePlayQueue'
import { useFilterChain } from '@/hooks/studio/useFilterChain'
import { useDJCommandContext } from '@/hooks/studio/useDJCommandContext'
...
export function ControlPanel() {
  ...
  const filterChain = useFilterChain(audioPlayer.audioContext)
  const quantize = useQuantize(audioPlayer.currentTrack?.bpm || 128)
  const loopRoll = useLoopRoll({
    audioElement: audioPlayer.audioElement,
    bpm: audioPlayer.currentTrack?.bpm || 128,
  })

  // Create DJ command context for AI control
  const djCommandContext = useDJCommandContext({
    audioPlayer,
    filterChain,
    quantize,
  })

  // Update quantize BPM when track changes
  useEffect(() => {
    if (audioPlayer.currentTrack?.bpm) {
      quantize.updateBPM(audioPlayer.currentTrack.bpm)
    }
  }, [audioPlayer.currentTrack?.bpm, quantize])
              ...
              <div className="bg-neutral-900 border border-neutral-700 rounded-lg p-4">
                ...
                {activeDJTab === 'autodj' && (
                  <div className="space-y-3">
                    ...
                    <CollapsibleSection title="AI Track Selection" defaultOpen={true} icon={<Bot className="w-5 h-5" />}>
                      <AutoDJPanel
                    ...
                    {/* Play Queue Panel */}
                    <CollapsibleSection title="Play Queue & History" icon={<Music className="w-5 h-5" />}>
                      <QueuePanel
                    ...
                    <CollapsibleSection title="Preset Modes" icon={<Settings className="w-5 h-5" />}>
                      <PresetModeSelector
                ...
                {activeDJTab === 'effects' && (
                  <div className="space-y-3">
                    {/* EQ & Filters Panel */}
                    <CollapsibleSection title="EQ & Filters" defaultOpen={true} icon={<Activity className="w-5 h-5" />}>
                      <EQPanel audioContext={audioPlayer.audioContext} />
                    </CollapsibleSection>

                    {/* DJ Filter Knobs (HPF/LPF) - NEW! */}
                    <CollapsibleSection title="Filter Knobs (HPF/LPF)" icon={<Filter className="w-5 h-5" />}>
                      <FilterKnobs
                        audioContext={audioPlayer.audioContext}
                        onHPFChange={filterChain.setHPF}
                        onLPFChange={filterChain.setLPF}
                        onReset={filterChain.reset}
                      />
                    </CollapsibleSection>

                    {/* Professional FX Chain */}
                    <CollapsibleSection title="FX Chain" icon={<Radio className="w-5 h-5" />}>
                      <FXChainPanel audioContext={audioPlayer.audioContext} />
                    </CollapsibleSection>

                    {/* Audio Effects */}
                    <CollapsibleSection title="Audio Effects" icon={<Music2 className="w-5 h-5" />}>
                      <AudioEffectsPanel
                    ...
                    {/* Crossfade Controls */}
                    <CollapsibleSection title="Crossfade Controls" icon={<Settings className="w-5 h-5" />}>
                ...
                {activeDJTab === 'performance' && (
                  <div className="space-y-3">
                    {/* Beat Matching Panel */}
                    <CollapsibleSection title="Beat Matching" defaultOpen={true} icon={<Activity className="w-5 h-5" />}>
                      <BeatMatchingPanel
                        currentTrack={audioPlayer.currentTrack}
                        isPlaying={audioPlayer.isPlaying}
                        playbackPosition={audioPlayer.playbackPosition}
                    ...
                    {/* Tempo & Key Lock Controls */}
                    <CollapsibleSection title="Tempo & Key Lock" icon={<Music className="w-5 h-5" />}>
                      <TempoKeyLockControls
                        tempo={audioPlayer.tempo}
                        isKeyLockEnabled={audioPlayer.isKeyLockEnabled}
                        onTempoChange={audioPlayer.setTempoRate}
                        onKeyLockToggle={audioPlayer.toggleKeyLock}
                        disabled={!audioPlayer.currentTrack}
                      />
                    </CollapsibleSection>

                    {/* Gain/Trim Controls */}
                    <CollapsibleSection title="Gain / Trim" icon={<BarChart3 className="w-5 h-5" />}>
                      <GainTrimControls
                        gain={audioPlayer.gain}
                        onGainChange={audioPlayer.changeGain}
                        disabled={!audioPlayer.currentTrack}
                      />
                    </CollapsibleSection>

                    {/* Quantize Controls */}
                    <CollapsibleSection title="Quantize" icon={<Activity className="w-5 h-5" />}>
                      <QuantizeControls
                        enabled={quantize.enabled}
                        snapToGrid={quantize.snapToGrid}
                        onToggle={quantize.toggle}
                        onGridChange={quantize.changeGrid}
                        disabled={!audioPlayer.currentTrack}
                      />
                    </CollapsibleSection>

                    {/* Loop Roll Controls */}
                    <CollapsibleSection title="Loop Roll" icon={<Disc className="w-5 h-5" />}>
                      <LoopRollControls
                        isActive={loopRoll.isActive}
                        activeLength={loopRoll.activeLength}
                        onToggle={loopRoll.toggleLoopRoll}
                        disabled={!audioPlayer.currentTrack}
                      />
                    </CollapsibleSection>

                    {/* Sound Effects Panel */}
                    <CollapsibleSection title="DJ Sound Effects" icon={<Radio className="w-5 h-5" />}>
                      <SoundEffectsPanel />
                    </CollapsibleSection>

                    {/* Professional Sampler Panel */}
                    <CollapsibleSection title="Sampler" icon={<Layers className="w-5 h-5" />}>
                      <ProfessionalSamplerPanel />
                    </CollapsibleSection>

                    {/* Recording Panel */}
                    <CollapsibleSection title="Recording" icon={<Mic className="w-5 h-5" />}>
                      <RecordingPanel
                        audioContext={audioPlayer.audioContext}
                    ...
                    {/* Vinyl Mode Panel */}
                    <CollapsibleSection title="Vinyl Mode" icon={<Disc className="w-5 h-5" />}>
                ...
                {activeDJTab === 'tools' && (
                  <div className="space-y-3">
                    ...
                    <CollapsibleSection title="AI Training System" defaultOpen={true} icon={<Brain className="w-5 h-5 text-purple-400" />}>
                      <AITrainingPanel />
                    </CollapsibleSection>

                    {/* AI Chat Control */}
                    <CollapsibleSection title="AI DJ Chat" icon={<Bot className="w-5 h-5" />}>
                      <AIChatPanel djCommandContext={djCommandContext} />
                    </CollapsibleSection>
                    ...
                    {/* Master Limiter */}
                    <CollapsibleSection title="Master Limiter" icon={<Shield className="w-5 h-5" />}>
                      <MasterLimiterControls
                        limiter={audioPlayer.limiter}
                        onThresholdChange={audioPlayer.setLimiterThreshold}
                        onRatioChange={audioPlayer.setLimiterRatio}
                        disabled={!audioPlayer.currentTrack}
                      />
                    </CollapsibleSection>
